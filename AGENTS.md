...
In `OUTPUT_RESULTS`, a Nextion press on component ID `2` triggers USB export. The code writes `RESULT.CSV` with this header:

```text
duty,effective_mV,torque_mNm,power_mW,rpm
```

A press on component ID `3` from the quit button returns the main state machine to setup.
...
5. `OUTPUT_RESULTS`
   ...
   - Component `2` exports CSV to USB.
   - Component `3` from the quit button returns to `WAIT_FOR_SETUP`.
